...
- **Conversation keeps the two sides on different languages.** Interpreting between a language and itself is nothing to do, and it is easy to arrive at: translating *into* English is an ordinary Simul setup, and switching back to conversation drops that English opposite the English source. Persisting the pair turned that from a transient glitch into a browser that came back English ⇄ English every load. Picking the other side's language now swaps the two; arriving at a clash any other way — a restore, a fallback to the first popular language, or turning Simul off — moves the target instead. Simul is exempt, since it is one-way and its source dropdown is hidden precisely because the value is ignored.
